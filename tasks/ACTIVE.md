...
Следующий кандидат: **ждёт команды автора** (НЕ начинать автоматически). Заморожено: M1–M26, S0.5, S1, S2, **S6 FROZEN (2026-09-14)**. S3/S4 — по своим DESIGN/report; S6 опирается на S3 §7 self-test contract и S4 claims/persistence. Открытые AD-вопросы: G2 user_confirmed (Q2), стоимость на уровне node НЕ вводить (решение S1: cost = backend+workflow). Будущие (вне scope S6): CLI-обёртка self-test — НЕ требуется (единственный entry = метод), real-E2E на живом ComfyUI для позитивной ветки, workflow→prompt конвертер для registry-графов. Детали: `docs/ECOSYSTEM_FIRST_S6_DESIGN.md`, `docs/ECOSYSTEM_FIRST_S6_IMPLEMENTATION_REPORT.md`.
